...
class Auth extends CI_Controller {
	...
	function manage_user($action = 'create', $id = null) {
		...
		if ($action == 'edit' && (!empty($id) OR $id == '0')) {			
			$user_data = (array) $this->ion_auth->user($id)->row();
			$user = array();
			...
			$user['user_groups'] = $this->ion_auth->get_users_groups($id)->result();
			$user['all_groups'] = $this->ion_auth->groups()->result();
		...
		if ($this->form_validation->run() == true && $action == 'update') {
			...
			$previous_groups = $this->ion_auth->get_users_groups($id)->result();
			foreach ($previous_groups as $previous_group) {
				
				// remove missing groups
				if(empty($new_groups[$previous_group->id])) {
					// remove this group
					$this->ion_auth->remove_from_group($previous_group->id, $id);
				} else {
			...
			if (!empty($new_groups)) {				
				...
				foreach ($new_group_ids as $new_group_id) {
					$this->ion_auth->add_to_group($new_group_id, $id);	
				}							
			}
